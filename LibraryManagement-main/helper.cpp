...
int InputValue(int maxlength, string& keyword, int& x, int& y, int type) {
	...
	if (type == 0) {
		...
		while (1) {
			...
			else if ((c > 47 && c < 58) || (c > 96 && c < 123) || c == SPACE) {
				if (len < maxlength) {
					if (c == 32) {
						if (len == 0 || keyword[len - 1] == SPACE) break;
					}
					if ((len == 0 || keyword[len - 1] == SPACE)  && c > 96 && c < 123){
						keyword.push_back(char(c - 32));
						PrintKeyWord(pattern, keyword, x, y);
					} else {
						keyword.push_back(char(c));
	...
	// type = 2 chu hoa dau moi chu
		...
		while (1) {
			...
			if (c == 224 || c == 0) {
			 ...
			 else if(c > 96 && c < 123 || c == SPACE){
				if(len < maxlength){
					if(c == SPACE){
						if (len == 0 || keyword[len - 1] == SPACE) break;
					}
					if (len == 0 || keyword[len - 1] == SPACE){
						keyword.push_back(char(c - 32));
						PrintKeyWord(pattern, keyword, x, y);
					} else {
						keyword.push_back(char(c));
